<template>
    <div>
        ...
        <div class="d-md-flex align-items-center mb-3">
            ...
            <div class="ml-auto">
                <noticia-modal-crear-editar-component @updateList="_updateList" :attrib="attrib" :asset="asset"></noticia-modal-crear-editar-component>
            </div>
        </div>
        <!--end breadcrumb-->
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="(item, key) in data.data" :key="key">
                                <th>
                                    <img :src="asset + 'storage/images_noticias/' + item.image" style="width: 100px;" v-if="item.image">
                                </th>
                                <td>{{ item.titulo }}</td>
                                <td>{{ item.link }}</td>
                                <td>
...
<script>
    export default {
        ...
        methods: {
            _getData(page = 1) {
                axios
                    .get("noticias/get-all-paginate?page=" + page, {
                        params: this.busqueda,
                    })
                    .then((response) => {
                        this.data = response.data.noticias;
                    })
                    .catch((err) => {
                        console.log(err);
                    });
